...
X_resp_lag <- as.vector(ts_EC) #Do this for each airline so 3 times
...
lambda_seq <- seq(2.5, 10, by = .1) #2.5, 10 best so far 
fit.glmnet <- cv.glmnet(pred_X, Y_EC, family="gaussian", intercept=TRUE, standardize.response=TRUE,alpha=1, lambda=lambda_seq)
...
ts.plot( ts_Test_fit, ts_EC[37:53], gpars = list(col = c("magenta","black")), ylab = "Profit")

#Plot of actual shifted along 4 quarters vs predicted
ts.plot( ts(ts_Test_fit[1:12]), ts_EC[42:53], gpars = list(col = c("magenta","black")), ylab = "Profit")



ser1 = Test_fit[2:12] - Test_fit[1:11]
ser2 = ts_EC[43:53] - ts_EC[42:52]
...
RMSE = sqrt(  1/length(ts_Test_fit[1:12]) * sum( (ts_EC[42:53] - ts_Test_fit[1:12])^2 )  )
# LASSO
# Ridge
...
MAE = 1/length(ts_Test_fit[1:12]) * sum(abs(ts_Test_fit[1:12] - ts_EC[42:53])) 
# LASSO
# Ridge
...
#Mean Squared Error (RMSE)
RMSE = sqrt(  1/length(ts_Test_fit) * sum( (ts_EC[37:53] - ts_Test_fit)^2 )  )
RMSE

#Mean Absolute Error (MAE)
MAE = 1/length(ts_Test_fit) * sum(abs(ts_Test_fit - ts_EC[37:53])) 
MAE
